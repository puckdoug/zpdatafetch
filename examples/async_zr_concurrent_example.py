...
async def fetch_concurrent(zwift_ids: list[int], zr: AsyncZR_obj) -> None:
...
async def fetch_with_limited_concurrency(
  ...
  print(
    f'Fetching {len(zwift_ids)} riders with {max_concurrent} max concurrent...',
  )
  riders = await asyncio.gather(
    *[fetch_one(zwift_id) for zwift_id in zwift_ids],
  )
...
async def fetch_using_batch(zwift_ids: list[int]) -> None:
