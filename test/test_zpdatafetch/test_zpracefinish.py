...
def test_zpracefinish_empty_instantiation():
  ...
  # Empty race finish includes all default field values
  result = obj.asdict()
  # Should have all fields with their default values
  assert result['category'] == ''
  assert result['category_women'] == ''
  assert result['time'] == 0.0
  assert result['time_hms'] == ''
  assert result['time_gun'] == 0.0
  assert result['time_gun_hms'] == ''
...
def race_finish(sample_race_data):
  ...
  return ZPRaceFinish.from_dict(sample_race_data)
...
class TestRaceFinishInitialization:
  ...
  def test_init_stores_data(self, sample_race_data):
    ...
    race = ZPRaceFinish.from_dict(sample_race_data)
    # Data should be properly parsed through from_dict
    # Excluded fields should be stored in _excluded
    ...
    for excluded_key in excluded_field_names:
      if excluded_key in sample_race_data:
        assert excluded_key in race._excluded, (
          f'{excluded_key} should be in _excluded'
        )

    # Field names should be mapped to dataclass attributes (not stored in _data)
    # The dataclass now has explicit fields instead of a _data dict
    field_mapping = {'ftp': 'zftp', 'zid': 'zwift_id', 'pos': 'position'}
    for orig_key, mapped_key in field_mapping.items():
      if orig_key in sample_race_data:
        # New name should be accessible as attribute
        attr_value = getattr(race, mapped_key, None)
        assert attr_value is not None or mapped_key == 'zftp', (
          f'{mapped_key} should be accessible as attribute'
        )

  def test_init_with_empty_dict(self):
    """Test initialization with empty dictionary."""
    race = ZPRaceFinish.from_dict({})
  ...
  def test_init_cleans_array_fields(self):
    ...
    race = ZPRaceFinish.from_dict(data)
    # Array fields should be cleaned to first element
    assert race.avg_power == 150
    assert race.avg_wkg == 3.0  # Converted to float
    assert race.weight == 50.0  # Converted to float
    # Field names should be mapped (zid→zwift_id), converted to int
  ...
  def test_init_handles_non_array_values(self):
    ...
    race = ZPRaceFinish.from_dict(data)
    # Non-array values should be converted/preserved
    assert race.avg_power == 150
    assert race.weight == 0.0  # None converted to default 0.0
...
class TestRaceFinishAttributeAccess:
  ...
  def test_getattr_returns_field_value(self, race_finish):
    ...
    assert (
      race_finish.event_title == 'Stage 1: Fresh Outta 25: Prospect Park Loop'
    )
    assert race_finish.position == 112
    assert (
      race_finish.zwift_id == 5230175
    )  # Field alias: zwid -> zwift_id, converted to int
  ...
  def test_getattr_returns_cleaned_values(self, race_finish):
    ...
    assert race_finish.avg_wkg == 2.5  # Converted to float
...
class TestRaceFinishRepr:
  ...
  def test_repr_contains_all_fields(self, race_finish):
    ...
    assert 'avg_power=123.0' in repr_str
  ...
  def test_repr_handles_empty_data(self):
    ...
    race = ZPRaceFinish.from_dict({})
    repr_str = repr(race)
    # Empty race includes all default field values
    assert 'ZPRaceFinish(' in repr_str
    assert 'zwift_id=0' in repr_str
    assert "event_date=''" in repr_str

  def test_str_contains_all_fields(self, race_finish):
    """Test str shows all race data fields."""
    ...
    assert 'avg_power=123.0' in str_repr

  def test_str_delegates_to_repr(self, race_finish):
    """Test that str delegates to repr for dataclass."""
    str_repr = str(race_finish)
    repr_repr = repr(race_finish)
    # For dataclasses, str should be same as repr
  ...
  def test_str_handles_empty_data(self):
    ...
    race = ZPRaceFinish.from_dict({})
    str_repr = str(race)
    # str should match repr for dataclass
    repr_repr = repr(race)
    assert str_repr == repr_repr
    assert 'ZPRaceFinish(' in str_repr
...
class TestRaceFinishAsdict:
  ...
  def test_asdict_returns_dict(self, race_finish):
    """Test that asdict returns a dictionary with all fields."""
    result = race_finish.asdict()
    # Should be a dict containing all explicit fields
...
class TestRaceFinishWithRealData:
  ...
  def test_real_race_data_has_expected_fields(self, real_race_data):
    ...
    race = ZPRaceFinish.from_dict(real_race_data)
