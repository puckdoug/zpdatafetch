...
class TestZRRiderFetch:
  ...
  def test_fetch_uses_epoch_parameter(self):
    ...
    with patch('zrdatafetch.zrrider.Config') as mock_config_class:
      ...
      with patch('zrdatafetch.zrrider.AsyncZR_obj') as mock_async_zr_class:
        mock_zr = MagicMock()
...
# ===============================================================================
class TestZRRiderRawAttribute:
  """Test that _raw attribute properly stores string data."""

  def test_raw_attribute_is_string_after_fetch(self):
    """Test that _raw is a string after fetch."""
    rider = ZRRider(zwift_id=123)
    ...
    with patch('zrdatafetch.zrrider.Config') as mock_config_class:
      ...
      with patch('zrdatafetch.async_zr.AsyncZR_obj.fetch_json') as mock_fetch:
        test_json = '{"name": "Test Rider", "gender": "M", "power": {"compoundScore": 250.0}, "race": {"current": {"rating": 2250.0, "mixed": {"category": "A"}}, "max30": {"rating": 2240.0, "mixed": {"category": "A"}}, "max90": {"rating": 2200.0, "mixed": {"category": "B"}}}}'
        mock_fetch.return_value = test_json

        rider.fetch()

        assert isinstance(rider._raw, str)
        assert rider._raw == test_json

  def test_raw_attribute_preserved_with_malformed_json(self):
    """Test that _raw preserves malformed JSON string."""
    rider = ZRRider(zwift_id=123)
    ...
    with patch('zrdatafetch.zrrider.Config') as mock_config_class:
      ...
      with patch('zrdatafetch.async_zr.AsyncZR_obj.fetch_json') as mock_fetch:
        ...
        # _raw should still contain the malformed string
        assert isinstance(rider._raw, str)
        assert rider._raw == malformed_json
        # Rider data should use defaults since parsing failed
        assert rider.name == 'Nobody'

  def test_raw_attribute_with_empty_response(self):
    """Test that _raw handles empty response."""
    rider = ZRRider(zwift_id=123)
  ...
  def test_parse_response_handles_malformed_json(self):
    """Test that _parse_response gracefully handles malformed JSON in _raw."""
    rider = ZRRider()
    rider._raw = '{this is not valid json}'
  ...
  def test_parse_response_handles_empty_raw(self):
    """Test that _parse_response handles empty _raw string."""
    rider = ZRRider()
    rider._raw = ''
  ...
  def test_parse_response_handles_whitespace_raw(self):
    """Test that _parse_response handles whitespace _raw string."""
    rider = ZRRider()
    rider._raw = '   \n  '
