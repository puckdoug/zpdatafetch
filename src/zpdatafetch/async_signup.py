"""Async version of the Signup class for fetching race signup lists."""
...
# ===============================================================================
class AsyncSignup(ZP_obj):
  """Async version of Signup class for fetching race signup lists.

  Retrieves lists of riders signed up for upcoming races asynchronously.

  Usage:
    async with AsyncZP() as zp:
      signup = AsyncSignup()
      signup.set_session(zp)
      await signup.fetch(3590800, 3590801)
      print(signup.json())

  Attributes:
    raw: Dictionary mapping race IDs to their signup lists
  """

  _url: str = 'https://zwiftpower.com/cache3/lists/'
  _url_end: str = '_zwift.json'

  def __init__(self) -> None:
    """Initialize a new AsyncSignup instance."""
    super().__init__()
  ...
  async def fetch(self, *race_id: int) -> dict[Any, Any]:
    """Fetch signup lists for one or more race IDs (async).
...
# ===============================================================================
async def main() -> None:
  """Example usage of AsyncSignup class."""
  parser = ArgumentParser(
    description='Fetch race signup lists from Zwiftpower (async)',
  )
  parser.add_argument(
    'race_ids',
    metavar='ID',
  ...
  async with AsyncZP() as zp:
    signup = AsyncSignup()
    signup.set_session(zp)
    await signup.fetch(*args.race_ids)
    print(signup.json())
